...
class DICOMViewer:
    ...
    def create_image_window(self):
        ...
        draw_line_button = tk.Button(image_window, text="Draw Line", command=lambda: self.activate_draw_line(ax, canvas))
        draw_line_button.pack(side=tk.LEFT, padx=5)
    ...
    def zoom_in(self, ax, canvas):
        ...
        ax.set_ylim(ax.get_ylim()[0] / 1.2, ax.get_ylim()[1] / 1.2)
        canvas.draw()
    def activate_draw_line(self, ax, canvas):
        # Activate the drawing tool to draw a straight line on the image
        ax.set_title("Draw a straight line (click and drag)")
        canvas.draw()

        # Connect the mouse click and release events to the drawing function
        cid_press = self.fig.canvas.mpl_connect("button_press_event", lambda event: self.on_press(event, ax, canvas))
        cid_release = self.fig.canvas.mpl_connect("button_release_event", lambda event: self.on_release(event, ax, canvas))

        # Store the connection IDs for later disconnection
        self.cid_press = cid_press
        self.cid_release = cid_release

    def on_press(self, event, ax, canvas):
    ...
    def on_release(self, event, ax, canvas):
        ...
        if hasattr(self, 'start_point'):
            ...
            ax.plot([self.start_point[0], end_point[0]], [self.start_point[1], end_point[1]], color='r', linestyle='-', linewidth=2)
            canvas.draw()
