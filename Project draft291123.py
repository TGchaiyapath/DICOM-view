...
class DICOMViewer:
    ...
    def load_dicom_tags(self, listbox):
        ...
        if self.file_paths:
            try:
                ...
                tags_to_include = ["PatientName", "PatientID", "StudyModality", "StudyTime"]
    ...
    def create_image_window(self):
        ...
        tags_window.title("DICOM Tags Viewer")

        # Create a Listbox to display DICOM tags
        tags_listbox = tk.Listbox(tags_window, selectmode=tk.BROWSE, height=20, width=40)
        tags_listbox.pack(padx=10, pady=10)

        # Load the DICOM tags in the Listbox
        ...
        canvas_widget.bind("<B1-Motion>", lambda event: self.on_drag(event, ax, canvas))
