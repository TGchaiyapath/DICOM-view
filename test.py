...
class DICOMViewer:
    def __init__(self, master):
        ...
        # Create image window content moved here
        ...
        self.toggle_drag_button = tk.Button(root, text="Toggle Drag", command=self.toggle_drag)
        
        
        # Next and Previous buttons in the new window
        next_button = tk.Button(root, text="Next", command=lambda: self.show_next(ax, canvas))
        
        root.bind("<Right>", lambda event: self.show_next(ax, canvas))
        prev_button = tk.Button(root, text="Previous", command=lambda: self.show_previous(ax, canvas))
        
        root.bind("<Left>", lambda event: self.show_previous(ax, canvas))
        ...
        zoom_in_button = tk.Button(root, command=lambda: self.zoom_in(ax, canvas), image=self.zoom_in_icon)
        
        root.bind("<Key-z>", lambda event: self.zoom_in(ax, canvas))
        ...
        # Zoom out button
        zoom_out_button = tk.Button(root, command=lambda: self.zoom_out(ax, canvas), image=self.zoom_out_icon)
        root.bind("<Key-x>", lambda event: self.zoom_out(ax, canvas))
        ...
        straight_line_button = tk.Button(root, command=lambda: self.activate_straight_line(ax, canvas), image=self.straight_icon)
        ...
        # Dash line button
        dashed_line_button = tk.Button(root, command=lambda: self.activate_dashed_line(ax, canvas), image=self.dashed_icon)
        ...
        #arrow
        single_arrow_button = tk.Button(root, command=lambda: self.activate_single_ended_arrow(ax, canvas),image=self.arrow_icon            )

        

        # Display the menu at the button location in the image window
        self.rotate_flip_button = tk.Button(root, text="Rotate/Flip", command=self.show_rotate_flip_menu)
        ...
        tags_listbox = Listbox(root, selectbackground='lightgray', selectmode=tk.SINGLE, height=10, width=30)
        ...
        #pack the widget
        self.browse_button.pack(side=tk.LEFT,anchor='nw', pady=10)
        self.hn_label.pack(side=tk.LEFT,anchor='nw', pady=10)
        self.hn_entry.pack(side=tk.LEFT,anchor='nw', pady=10)
        self.load_button.pack(side=tk.LEFT,anchor='nw', pady=10)
        self.error_label.pack(side=tk.LEFT,anchor='nw', pady=10)
    ...
    def load_dicom_image(self):
        ...
        if hn:
            ...
            if self.file_paths:
                ...
                # Load the DICOM image into the canvas
    ...
    def show_rotate_flip_menu(self):
        ...
        rotate_flip_menu = Menu(self.master)

        
        rotate_flip_menu.add_command(label="Rotate Clockwise", command=lambda: self.rotate_image(clockwise=True))
        rotate_flip_menu.add_command(label="Rotate Counterclockwise", command=lambda: self.rotate_image(clockwise=False))
        rotate_flip_menu.add_command(label="Flip Horizontal", command=lambda: self.flip_image(horizontal=True))
        rotate_flip_menu.add_command(label="Flip Vertical", command=lambda: self.flip_image(vertical=True))
