...
class DICOMViewer:
    def __init__(self, master):
        ...
        self.accession_number = ttk.StringVar()  # Variable to store Accession number
        self.hospital_number = ttk.StringVar()  # Variable to store Hospital Number
        ...
        self.hn_label = ttk.Label(self.master, text="HN (Hospital Number):")
        self.hn_label.pack(pady=5)

        self.hn_entry = ttk.Entry(self.master, textvariable=self.hospital_number, width=20)
        self.hn_entry.pack(pady=5)
        self.hn_entry.bind("<Return>", self.load_dicom_from_directory)

        self.accession_label = ttk.Label(self.master, text="Accession:")
        self.accession_label.pack(pady=5)

        self.accession_entry = ttk.Entry(self.master, textvariable=self.accession_number, width=20)
        self.accession_entry.pack(pady=5)

        # Load button
        self.load_button = ttk.Button(self.master, text="Load DICOM", command=self.load_dicom_image)
    ...
    def create_image_window(self):
        ...
        image_window = ttk.Toplevel(self.master)
        ...
        info_frame.pack(side=ttk.RIGHT, padx=10, pady=10, fill=ttk.Y)

        # Add labels for DICOM information
        info_label = ttk.Label(info_frame, text="DICOM Information", font=('Helvetica', 12, 'bold'))
        ...
        self.toggle_drag_button = ttk.Button(image_window, text="Toggle Drag", command=self.toggle_drag)
        

        # Next and Previous buttons in the new window
        next_button = ttk.Button(image_window, text="Next", command=lambda: self.show_next(ax, canvas))
        image_window.bind("<Right>", lambda event: self.show_next(ax, canvas))
        prev_button = ttk.Button(image_window, text="Previous", command=lambda: self.show_previous(ax, canvas))
        ...
        icon_ZM = ttk.PhotoImage(file=ZM_icon_path)
        self.zoom_in_icon = icon_ZM.subsample(27, 27)

        # Zoom in button with icon
        zoom_in_button = ttk.Button(image_window, command=lambda: self.zoom_in(ax, canvas), image=self.zoom_in_icon, )
        ...
        icon_ZO = ttk.PhotoImage(file=ZO_icon_path)
        self.zoom_out_icon = icon_ZO.subsample(27, 27)
        #Zoom out button
        zoom_out_button = ttk.Button(image_window, command=lambda: self.zoom_out(ax, canvas),image=self.zoom_out_icon)
        ...
        icon_ST = ttk.PhotoImage(file=straight_icon_path)
        self.straight_icon = icon_ST.subsample(27, 27)
        # Add drawing tool button in the new window
        straight_line_button = ttk.Button(image_window, command=lambda: self.activate_straight_line(ax, canvas),image=self.straight_icon)
    

        # Load a dashe icon image
        dashed_icon_path = os.path.abspath(r"C:\Users\Admin\Desktop\TG\TEST\icon\dashed line.png")  
        icon_D = ttk.PhotoImage(file=dashed_icon_path)
        self.dashed_icon = icon_D.subsample(27, 27)
        #dash line button
        dashed_line_button = ttk.Button(image_window, command=lambda: self.activate_dashed_line(ax, canvas),image=self.dashed_icon)

        # Load a dashe icon image
        arrow_icon_path = os.path.abspath(r"C:\Users\Admin\Desktop\TG\TEST\icon\up-right-arrow.png")  
        icon_AR = ttk.PhotoImage(file=arrow_icon_path)
        self.arrow_icon = icon_AR.subsample(27, 27)
        #arrow
        single_arrow_button = ttk.Button(image_window, command=lambda: self.activate_single_ended_arrow(ax, canvas),image=self.arrow_icon            )
        

        # Display the menu at the button location in the image window
        self.rotate_flip_button = ttk.Button(image_window, text="Rotate/Flip", command=self.show_rotate_flip_menu)
        contrast_button = ttk.Button(image_window, command=lambda: self.contrast_adjustment(ax, canvas),text="contrast")
        ...
        #place widjet
        self.canvas_widget.pack(side=ttk.BOTTOM, expand=True, padx=5, pady=5,anchor="sw",fill="x")
        self.toggle_drag_button.pack(side=ttk.LEFT, padx=5, pady=5,anchor="nw",fill="both",expand=False)
        next_button.pack(side=ttk.LEFT, padx=5, pady=5,anchor="nw",fill="both",expand=False)
        prev_button.pack(side=ttk.LEFT, padx=5, pady=5,anchor="nw",fill="both",expand=False)
        zoom_in_button.pack(side=ttk.LEFT, padx=5, pady=5,anchor="nw",fill="both",expand=False)
        zoom_out_button.pack(side=ttk.LEFT, padx=5, pady=5,anchor="nw",fill="both",expand=False)
        straight_line_button.pack(side=ttk.LEFT, padx=5, pady=5,anchor="nw",fill="both",expand=False)
        dashed_line_button.pack(side=ttk.LEFT, padx=5, pady=5,anchor="nw",fill="both",expand=False)
        single_arrow_button.pack(side=ttk.LEFT, padx=5, pady=5,anchor="nw",fill="both",expand=False)
        self.rotate_flip_button.pack(side=ttk.LEFT, padx=5, pady=5,anchor="nw",fill="both",expand=False)
        contrast_button.pack(side=ttk.LEFT, padx=5, pady=5,anchor="nw",fill="both",expand=False)
    ...
    def load_dicom_image_internal(self, ax, canvas):
        if self.file_paths:
            try:
                ...
                ax.clear()
                ax.imshow(dicom_data.pixel_array, cmap=plt.cm.gray)  # Set aspect='auto'
                ax.axis('off')  # Hide the axes
                canvas.draw()
    ...
    def update_entry(self):
        ...
        self.accession_entry.delete(0,ttk.END)
        self.accession_entry.insert(ttk.END, self.accession_number.get())

        self.hn_entry.delete(0, ttk.END)
        self.hn_entry.insert(ttk.END, self.hospital_number.get())
            ...
            try:
                ...
                for tag in tags_to_include:
                    if tag in dicom_data:
                        listbox.insert(ttk.END, f"{tag}: {dicom_data[tag].value}")
